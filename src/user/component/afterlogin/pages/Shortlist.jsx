...
const Shortlist = () => {
  ...
  useEffect(() => {
    if (user) {
      setShortlistItems(getShortlistForUser(user.id))
    }
  }, [user])

  const handleRemove = (id) => {
    if (!user) return
    removeFromShortlist(user.id, id)
    setShortlistItems(getShortlistForUser(user.id))
    refreshData()
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{shortlistItems.length}</p>
      ...
      {shortlistItems.length > 0 ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {shortlistItems.map((profile) => (
            <div key={profile.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all">
              ...
              <div className="p-4">
                <Link to={`/profile/${profile.id}`} className="block">
                  ...
                  <p className="text-gray-500 text-sm mb-3">{profile.age} years old</p>
                </Link>
                
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <Briefcase className="w-4 h-4 text-gray-400" />
                    <span>{profile.education?.occupation || profile.profession}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-600">
        ...
        <div className="bg-white rounded-2xl p-12 shadow-sm border border-gray-100 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No Shortlisted Profiles</h3>
          <p className="text-gray-500 mb-6">Start exploring profiles and shortlist your favorites</p>
          <Link
            to="/afterlogin/search"
