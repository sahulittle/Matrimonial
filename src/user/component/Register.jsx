            ...
            <form className="mt-8 space-y-6" onSubmit={(e) => { e.preventDefault(); setStep(2); }}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="mt-1 relative"><select id="education" name="education" required onChange={handleChange} value={formData.education || ''} className="appearance-none block w-full px-3 py-3 border border-gray-300 text-gray-900 rounded-md focus:outline-none focus:ring-pink-500 focus:border-pink-500 sm:text-sm pr-8"><option value="" disabled selected>Select Education</option>{educationOptions.map(edu => <option key={edu} value={edu}>{edu}</option>)}</select><div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700"><FaChevronDown className="h-4 w-4" /></div></div>
                </div>
