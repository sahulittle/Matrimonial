...
const IgnoredProfile = () => {
    ...
    // Mock data for 100 ignored profiles
    const ignoredProfiles = Array.from({ length: 100 }, (_, i) => {
        const userName = names[i];
        const profileName = names[(i + 50) % 100]; // Just some other user
    ...
    const currentIgnored = filteredIgnored.slice(indexOfFirstUser, indexOfLastUser);
                    ...
                    <tbody className="text-gray-700">
                        {currentIgnored.map((item, index) => (
                            <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td className="py-3 px-4"><div className="flex items-center"><img src={item.user.avatar} alt={item.user.name} className="w-10 h-10 rounded-full mr-4" /><div><p className="font-medium text-gray-800">{item.user.name}</p><p className="text-sm text-gray-500">@{item.user.username}</p></div></div></td>
                                <td className="py-3 px-4"><div className="flex items-center"><img src={item.profile.avatar} alt={item.profile.name} className="w-10 h-10 rounded-full mr-4" /><div><p className="font-medium text-gray-800">{item.profile.name}</p><p className="text-sm text-gray-500">@{item.profile.username}</p></div></div></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
