...
const KycUnverified = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const res = await getAllUsers({
                status: "kycUnverified",
                search: searchTerm,
            });
            setUsers(res.users || []);
        } catch (error) {
            console.error("Error fetching KYC unverified users:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredUsers = users; // already filtered from backend

    return (
        <div className="bg-white p-8 rounded-lg shadow-md">
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                <h4 className="text-2xl font-bold text-gray-800">KYC Unverified Users</h4>
