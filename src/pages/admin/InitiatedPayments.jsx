  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        console.error("Error loading initiated payments:", err);
      }
    };
    fetch();
  }, []);
  ...
  return (
    <div className="bg-white p-4 md:p-8 rounded-lg shadow-md">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        <h4 className="text-xl md:text-2xl font-bold text-gray-800">Initiated Payments</h4>
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {currentPayments.length > 0 ? (
          currentPayments.map((payment) => {
            const initiatedTime = formatInitiated(payment.createdAt || payment.initiated);
            const user = payment.user || payment.userId || {};
            return (
              <div key={payment.id || payment._id} className="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center">
                    <img 
                      src={user.avatar || user.profilePhoto || "/default-avatar.jpg"} 
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm border-t border-gray-100 pt-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">{payment.gateway?.toUpperCase() || payment.paymentMethod?.toUpperCase() || "CCAVENUE"}</p>
                  </div>
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="min-w-full bg-white">
          ...
          <tbody className="text-gray-700">
            {currentPayments.length > 0 ? (
              currentPayments.map((payment) => {
                const initiatedTime = formatInitiated(payment.initiated || payment.createdAt);
                const user = payment.user || payment.userId || {};
                return (
                  <tr
                  ...
                  >
                    <td className="py-3 px-4">
                      <p className="font-medium text-gray-800">
                        {payment.gateway?.toUpperCase() || payment.paymentMethod?.toUpperCase() || "CCAVENUE"}
                      </p>
              ...
              <tr>
                <td colSpan="6" className="text-center py-12 text-gray-500 italic">
                  No initiated payments found matching your criteria.
                </td>
              </tr>
            )}
          </tbody>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 py-3 gap-1">
                ...
                <p className="text-gray-800 font-medium sm:text-right">
                  {selectedPayment.userId?.fullName || selectedPayment.userId?.name}
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 py-3 gap-1">
                ...
                <p className="text-gray-800 font-medium sm:text-right">
                  {selectedPayment.gateway?.toUpperCase() || selectedPayment.paymentMethod?.toUpperCase()}
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 py-3 gap-1">
                ...
                <p className="text-gray-800 font-medium sm:text-right">
                  {new Date(selectedPayment.initiated || selectedPayment.createdAt).toLocaleString()}
                </p>
              </div>
              ...
              {Object.entries(selectedPayment.details || {}).map(
                ([key, value]) => (
                  <div key={key} className="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 py-3 gap-1">
                    ...
                    <p className="text-gray-800 font-medium sm:text-right break-all">{String(value)}</p>
                  </div>
                ),
              )}
            </div>
...
export default InitiatedPayments;
