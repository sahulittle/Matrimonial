...
const AllPayments = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await getAllPayments({ page: 1, limit: 200 });
  ...
  const paymentStats = useMemo(() => {
    const stats = {
      successful: {
        ...
        icon: <FiCheckCircle />,
        color: "green",
        link: "/admin/payments/successful",
      },
      pending: {
        total: 0,
        count: 0,
      ...
      initiated: {
        ...
        icon: <FiLoader />,
        color: "blue",
        link: "/admin/payments/initiated",
      },
    };
    payments.forEach((p) => {
      const status = (p.status || "").toLowerCase();
      if (status === "success" || status === "successful") {
        stats.successful.total += p.amount || 0;
        stats.successful.count++;
      } else if (status === "pending") {
        stats.pending.total += p.amount || 0;
        stats.pending.count++;
      } else if (status === "failed" || status === "rejected") {
        stats.rejected.total += p.amount || 0;
        stats.rejected.count++;
      } else if (status === "initiated") {
        stats.initiated.total += p.amount || 0;
        stats.initiated.count++;
      }
    });
    return stats;
  }, [payments]);
    ...
    () =>
      payments.filter((payment) => {
        ...
        const paymentDate = new Date(
          payment.createdAt || payment.initiated || payment.createdAt,
        );
          ...
          <table className="min-w-full bg-white">
            ...
            <tbody className="text-gray-700">
              {currentPayments.map((payment) => {
                const initiatedTime = formatInitiated(
                  payment.createdAt || payment.initiated,
                );
                const statusLabel =
