...
import { FiGrid, FiUsers, FiPackage, FiBarChart2, FiSettings, FiChevronDown } from 'react-icons/fi';
...
const Sidebar = () => {
  ...
  const navLinks = [
    ...
    { to: '/admin/system-setting', icon: <FiSettings size={20} />, text: 'System Setting' },
    {
        text: 'Report',
  ...
  useEffect(() => {
    const activeDropdown = navLinks.find(link => link.submenu && isSubmenuActive(link.submenu));
    if (activeDropdown) {
      setOpenDropdown(activeDropdown.text);
    }
  }, [location.pathname]);

  const baseLinkClasses = "flex items-center px-6 py-3 text-gray-300 rounded-lg mx-3 my-1 transition-colors duration-200";
