...
export function getConversationsForUser(userId) {
  ...
  // New format: { [userId]: Conversation[] }
  if (raw && !Array.isArray(raw) && typeof raw === 'object') {
    const list = raw[userId];
    return Array.isArray(list) ? list : [];
  }

  // Legacy format: Conversation[] with { users: [idA, idB], lastMessage: string }
  if (Array.isArray(raw)) {
    return raw
      .filter((conv) => Array.isArray(conv?.users) && conv.users.includes(String(userId)))
      .map((conv) => {
        const otherUserId = conv.users.find((id) => String(id) !== String(userId));
        return {
          id: conv.id,
          userId: otherUserId,
          lastMessage:
