...
function ensureCoreSeedUsers() {
  ...
  const hasAdmin = users.some(
    (u) => String(u.email || "").toLowerCase() === "admin@matrilab.com"
  );
  const hasDemoClient = users.some(
    (u) => String(u.email || "").toLowerCase() === DEMO_USER_EMAIL
  );
...
export function initializeDemoData() {
  ...
  if (demoInitialized) {
    ...
    if (valid) {
      // Keep existing local data but guarantee core seed accounts are available.
      ensureCoreSeedUsers();
  ...
  // Guard against stale/partial user data from previous versions.
  ensureCoreSeedUsers();
    ...
    "1": [
      {
        id: "conv1",
        userId: "3",
        lastMessage: { text: "Hi! Nice to connect with you." },
