...
const Reports = () => {
    ...
    const reportTitles = ["Inappropriate Photo", "Spamming", "Fake Profile", "Harassment", "Scamming"];

    // Mock data for 100 reports
    const reports = Array.from({ length: 100 }, (_, i) => {
        ...
        return {
            ...
            details: `This is a detailed description of the report against ${profileName} by ${userName}. The issue is regarding ${reportTitles[i % reportTitles.length]}. Please investigate further.`
        };
    });

    const filteredReports = reports.filter(report =>
        report.user.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        report.profile.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const currentReports = filteredReports.slice(indexOfFirstUser, indexOfLastUser);
    ...
    const handleBanSubmit = (e) => {
        e.preventDefault();
        console.log(`Banning user: ${selectedReport.profile.name} for reason: ${banReason}`);
        closeModal();
    };
    ...
    return (
        <div className="bg-white p-8 rounded-lg shadow-md">
                    ...
                    <tbody className="text-gray-700">
                        {currentReports.map((report, index) => (
                            <tr key={report.id} className="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td className="py-3 px-4"><div className="flex items-center"><img src={report.user.avatar} alt={report.user.name} className="w-10 h-10 rounded-full mr-4" /><div><p className="font-medium text-gray-800">{report.user.name}</p><p className="text-sm text-gray-500">@{report.user.username}</p></div></div></td>
                                <td className="py-3 px-4"><div className="flex items-center"><img src={report.profile.avatar} alt={report.profile.name} className="w-10 h-10 rounded-full mr-4" /><div><p className="font-medium text-gray-800">{report.profile.name}</p><p className="text-sm text-gray-500">@{report.profile.username}</p></div></div></td>
                                <td className="py-3 px-4">{report.title}</td>
                                <td className="py-3 px-4 text-center">
                                    <div className="flex justify-center items-center gap-2">
                                        <button onClick={() => openDetailsModal(report)} className="flex items-center bg-blue-500 text-white px-3 py-1 rounded-md text-sm hover:bg-blue-600"><FiEye className="mr-1" /> Details</button>
                                        <button onClick={() => openBanModal(report)} className="flex items-center bg-red-500 text-white px-3 py-1 rounded-md text-sm hover:bg-red-600"><FiSlash className="mr-1" /> Ban</button>
                                    </div>
                                </td>
                            </tr>
                        ))}
            ...
            {isDetailsModalOpen && selectedReport && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg"><div className="flex justify-between items-center mb-4 border-b pb-4"><h4 className="text-xl font-bold text-gray-800">Report Details</h4><button onClick={closeModal} className="text-gray-500 hover:text-gray-800"><FiX size={24} /></button></div><div><h5 className="font-bold text-lg mb-2">{selectedReport.title}</h5><p className="text-gray-600 mb-2"><strong>Reported by:</strong> {selectedReport.user.name} (@{selectedReport.user.username})</p><p className="text-gray-600 mb-4"><strong>Reported profile:</strong> {selectedReport.profile.name} (@{selectedReport.profile.username})</p><p className="text-gray-700">{selectedReport.details}</p></div><div className="mt-6 flex justify-end"><button type="button" onClick={closeModal} className="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-300">Close</button></div></div></div>
            )}

            {/* Ban Modal */}
            {isBanModalOpen && selectedReport && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md"><div className="flex justify-between items-center mb-4"><h4 className="text-xl font-bold text-gray-800">Ban User</h4><button onClick={closeModal} className="text-gray-500 hover:text-gray-800"><FiX size={24} /></button></div><form onSubmit={handleBanSubmit}><p className="text-gray-600 mb-4">If you ban this user ({selectedReport.profile.name}), he/she won't be able to access his/her dashboard.</p><div><label htmlFor="banReason" className="block text-sm font-medium text-gray-700 mb-1">Reason</label><textarea id="banReason" name="banReason" value={banReason} onChange={(e) => setBanReason(e.target.value)} required className="w-full h-24 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500" rows="4" /></div><div className="mt-6 flex justify-end"><button type="button" onClick={closeModal} className="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg mr-4 hover:bg-gray-300">Cancel</button><button type="submit" className="bg-red-600 text-white px-6 py-2 rounded-lg shadow-md hover:bg-red-700">Submit</button></div></form></div></div>
            )}
        </div>
    );
};
